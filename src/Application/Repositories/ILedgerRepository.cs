...
public interface ILedgerRepository
{
    ...
    ///
    /// What a new book starts with comes from <see cref="Chart.StandardAccounts"/>.
    /// </summary>
    ...
    /// <summary>
    /// Records a fact rather than a statement — money changing hands. Converging one would CORRECT
    /// it if the figure upstream had moved, and a payment that happened is reversed, not restated,
    /// so the first entry under a source wins and later ones write nothing.
    /// </summary>
    Task<bool> RecordOnceAsync(JournalEntry candidate, CancellationToken ct = default);
